...
public class OrderCreationUseCaseTest
{
    ...
    public void SellMultipleItems()
    {
        ...
        Assert.Equal(2, insertedOrder.Items[0].Quantity.Value);
        ...
        Assert.Equal(3, insertedOrder.Items[1].Quantity.Value);
    ...
    public void UnknownProduct()
    {
        var request = new SellItemsRequest
        {
            Requests = new List<SellItemRequest> { new() { ProductName = "unknown product"} }
        };

        void ActionToTest() => _useCase.Run(request);

        Assert.Throws<UnknownProductException>((Action)ActionToTest);
    }
}